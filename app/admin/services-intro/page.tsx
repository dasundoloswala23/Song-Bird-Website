...
import { DEFAULT_SERVICES_INTRO } from '@/components/ServicesIntro'
...
export default function AdminServicesIntroPage() {
  const [form, setForm] = useState<ServicesIntroDoc>(DEFAULT_SERVICES_INTRO)
  ...
  useEffect(() => {
    import('@/lib/firestorePublic').then(({ getServicesIntro }) =>
      getServicesIntro().then(d => { if (d) setForm(d); setLoading(false) })
    )
  }, [])
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      const { saveServicesIntro } = await import('@/lib/firestorePublic')
      await saveServicesIntro(form)
      setToast('Saved!')
      ...
      <form onSubmit={onSubmit}>
        <div className="bg-navy-card border border-gold-brushed/15 rounded-xl p-6 space-y-4">
          <div>
            ...
            <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} className={inp} placeholder="One Firm. Every Path." />
          </div>
          <div>
            <label className="block text-[11px] font-sans font-semibold uppercase tracking-[0.15em] text-gold-brushed mb-1.5">Body</label>
            <textarea value={form.body} onChange={e => setForm({ ...form, body: e.target.value })} rows={10} className={inp + ' resize-y'} placeholder="Songbird delivers an integrated ecosystem…" />
          </div>
        </div>
