...
function subjectFor(d: LeadEmailData): string {
  switch (d.type) {
    case "consultation":
      return `New Consultation Request — ${d.name ?? "Website"}`;
    case "eligibility":
      return `New Eligibility Submission — ${d.name ?? "Website"}`;
    case "booking":
      return `New Consultation Booking — ${d.name ?? "Website"}${d.date ? ` — ${d.date} ${d.startTime ?? ""}` : ""}`;
    default:
      return `New Website Inquiry — ${d.name ?? "Website"}`;
  }
}
...
function buildHtml(d: LeadEmailData): string {
  const sessionRow = d.sessionType
    ? row(
        "Session",
        d.charge ? `${d.sessionType} — ${d.charge}` : d.sessionType,
      )
    : "";
  const cvRow = d.cvUrl
    ? `<tr>
