    ...
    <section className="py-24 bg-cream" aria-labelledby="process-heading">
      <div className="mx-auto px-6 md:px-12 max-w-7xl">
        <div className="text-center mb-16">
          ...
          <h2 id="process-heading" className="font-serif font-normal text-[38px] md:text-[48px] leading-tight text-ink">
            {sectionTitle}
          </h2>
...
function StepCard({ step, num }: { step: { title: string; description: string }; num: string }) {
  ...
  return (
    ...
    >
      ...
      >
        <span className="font-serif font-normal text-[18px] text-white select-none">{num}</span>
      </div>
      <h3 className="font-serif font-normal text-[17px] text-ink mb-2 leading-snug">{step.title}</h3>
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: NODE_GRADIENT }}
      >
        <span className="font-serif font-normal text-[14px] text-white select-none">{num}</span>
      </div>
      <div className="pt-1 flex-1 min-w-0">
        <h3 className="font-serif font-normal text-[17px] text-ink mb-1.5 leading-snug">{step.title}</h3>
        <p className="text-[13px] font-sans text-slate leading-relaxed">{step.description}</p>
      </div>
    </div>
  )
